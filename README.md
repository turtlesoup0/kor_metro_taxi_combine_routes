...
3. 전략별 Dijkstra 탐색
   - S1: 대중교통 전용
   - S2: 택시→대중교통 (first-mile)
   - S3: 대중교통→택시 (last-mile)
   - S4: 택시→대중교통→택시
   - S5: 대중교통→택시→대중교통 (mid-route 환승 택시 치환)

4. 불합리 경로 필터링
   - 택시 시간 절약 ≤5분 → 제거
   - 분당 택시비 >1,500원 → 제거
   - 다구간 분할 택시 (택시-택시-택시) → 제거
   - 경로 gap 자동 연결 leg 삽입
...
상세 현황은 [docs/PROJECT_STATUS.md](docs/PROJECT_STATUS.md) 참조.

**주요 이슈:**
- ODsay API 호출량 제한 (1회 검색 30~40회 → 429 에러 발생 가능)
- 택시 구간 대중교통 교차검증 미완성 (API 제한으로 후반 검증 실패)
- 카카오 모빌리티 대중교통 API 등 대안 API 검토 필요
